import { signInWithEmailAndPassword, GoogleAuthProvider, FacebookAuthProvider, signInWithCredential, OAuthProvider } from 'firebase/auth';
...
export const loginWithGoogle = async (): Promise<{ user: FirestoreUser; isNewUser: boolean }> => {
  try {
    ...
    if (error.code === 'auth/popup-closed-by-user' || error.code === 'auth/cancelled-popup-request' || error.message === 'Autenticación cancelada') {
