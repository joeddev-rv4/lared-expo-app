...
class WhatsAppService {
  ...
  constructor() {
    ...
    if (apiKey) {
      ...
      console.log('⚠️ OPENAI_API_KEY no configurado, usando mensajes estáticos');
    }
  }
  ...
  private async generateApiKey(): Promise<string> {
    ...
    if (!email || !password) {
      throw new Error('LARED_API_EMAIL and LARED_API_PASSWORD must be set in environment variables');
    }

    console.log("📡 Llamando a generate-api-key...");
    const response = await fetch('https://panel.laredgt.com/api/user/generate-api-key', {
      method: 'POST',
      headers: {
  ...
  async sendMessage(phoneNumber: string, message: string): Promise<SendMessageResponse> {
    try {
      ...
      const response = await fetch('https://panel.laredgt.com/notifications/send-message', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          ...
          const retryResponse = await fetch('https://panel.laredgt.com/notifications/send-message', {
            method: 'POST',
            headers: {
          ...
          if (!retryResponse.ok) {
            ...
            console.error("❌ Error en segundo intento:", retryResponse.status, retryErrorText);
            throw new Error(`Failed to send message: ${retryResponse.statusText}`);
          }
    ...
    try {
      ...
      const completion = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'Eres un asistente que genera mensajes de verificación creativos y amigables para WhatsApp. Los mensajes deben ser cortos (máximo 3 líneas), profesionales pero cálidos, y siempre incluir el código de verificación de forma clara. Varía el estilo y las palabras en cada mensaje para que nunca sean iguales.'
          },
          {
            role: 'user',
